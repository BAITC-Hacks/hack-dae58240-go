...
function column(header, re, label, file, report, required = true) {
  ...
  if (index < 0) {
    ...
    (required ? report.errors : report.notes).push(message); // необязательная колонка — справка, не предупреждение
...
function parseSeasonality(file, report) {
  ...
  } else report.notes.push(`В файле «${path.basename(file)}» не найдена колонка «СЕЗОННОСТЬ» — сезонность будет рассчитана по продажам или полным годам.`);
...
function loadAll() {
  ...
  for (const item of report) {
    ...
    else if (!salesMonths.has(asOfMonth) || !stockMonths.has(asOfMonth)) item.warnings.push('Месячные таблицы не содержат месяц даты актуальности — прогноз может быть неполным.');
    ...
    for (const note of item.notes || []) console.log(`[DATA CHECK] ℹ️ ${note}`);
  }
