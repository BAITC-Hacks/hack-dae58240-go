...
// Порог разовой продажи: строка накладной больше max(Q3 + 3·IQR, 5·медиана строки, типичный месяц).
// Такая строка заменяется медианной строкой — излишек не идёт в регулярный спрос.
function detectOneOffs(lines, typicalMonth, fromMonth, lastMonth) {
  ...
  const q = window.map(l => l.qty);
  const q1 = quantile(q, 0.25), q3 = quantile(q, 0.75), med = quantile(q, 0.5);
  // при короткой истории (<6 строк) статистика ненадёжна — порог только от типичного месяца
  let threshold = window.length < 6 ? Math.max(3 * typicalMonth, 5) : Math.max(q3 + 3 * (q3 - q1), 5 * med, typicalMonth, 5);
  // если «крупных» накладных много — это регулярные оптовые клиенты, а не разовые события:
  // тогда исключаем только совсем аномальные (вдвое выше порога)
  if (window.filter(l => l.qty > threshold).length > 4) threshold *= 2;
  const events = window.filter(l => l.qty > threshold)
    .map(l => ({ date: l.date, doc: l.doc, qty: l.qty, excluded: r1(l.qty - med) })); // заменяем типичной строкой
...
function computeSku(P, k, { horizonDays = 30, growthPct = 0, overrides = {} } = {}) {
  ...
  const urgency = daily <= 0 ? 'low' : stock <= 0 || coverDays < L ? 'high' : coverDays < L + horizonDays ? 'medium' : 'low';

  const flags = [];
  if (stock <= 0 && daily > 0) flags.push('stockout_now');
