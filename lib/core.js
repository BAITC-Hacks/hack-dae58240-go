...
function prepare(supplier) {
  ...
  for (const [code, s] of Object.entries(m.sales)) {
    ...
    const dataCategory = String(m.transit[code]?.category || '').trim();
    skus[code] = {
      code, name: s.name.trim(), category: dataCategory || categoryOf(s.name),
      categorySource: dataCategory ? 'данные' : 'по наименованию', series,
  ...
  const catSeason = {};
  for (const [c, list] of Object.entries(byCat)) if (list.length >= 3 && sum(list.map(s => sum(Object.values(s)))) >= 500) catSeason[c] = seasonalIndex(list, months);
...
// noOutlierFilter, noStockoutAdj, noSeasonality, noCategory, noTrend
function computeSku(P, k, { horizonDays = 30, growthPct = 0, overrides = {} } = {}) {
  ...
  const categorySeason = !o.noCategory && P.catSeason[k.category];
  const baseSeason = categorySeason || P.brandSeason;
  const seasonSource = categorySeason ? `категория «${k.category}»` : `производитель (${P.brandSeasonSource})`;
  ...
  return {
    sku: k.code, name: k.name, supplier: P.supplier, category: k.category, categorySource: k.categorySource, abc: k.abc,
...
function analyze_sku(args) {
  ...
  return {
    ...
    experiments: {
      ...
      withoutCategory: v(q({ noCategory: true })),
